...
def main(train_path,test_path):
    ...
    test_features, _ = preprocess.preprocess(SQL, test_path, pipelineModel=pmodel)
    print("Preprocessing done. Feature count =", test_features.count())

    # 3) Evaluate
    results = evaluate.evaluate(test_features, model, threshold)
    print("Evaluation:\n")
    print("Anomaly count:", results["anomaly_count"])
    print("Sample anomalies:", results["sample"])
