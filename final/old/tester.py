...
test_path = "hdfs:///user/kingsmen/kdd_1999/kddcup.testdata.unlabel_10_percent.csv"
lines = sql.read.format("com.databricks.spark.csv").option("inferSchema", "true").option("header", "false").load(data_path)
test_lines = sql.read.format("com.databricks.spark.csv").option("inferSchema", "true").option("header", "false").load(test_path)
...
test_lines = test_lines.toDF(*cols)

lines = lines.drop("label")
test_lines = test_lines.drop("label")
...
df2 = preproc_model.transform(test_lines)
